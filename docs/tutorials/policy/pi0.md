...
```bash
cd policy/pi0
sudo apt install -y git-lfs build-essential pkg-config
conda install -c conda-forge -y "ffmpeg>=7,<8" pkg-config cython
GIT_LFS_SKIP_SMUDGE=1 uv sync
```

## Generate RoboSynChallenge Data
See <a href="../collect_data.html">RoboSynChallenge Tutorial (Collect Data Section)</a> for more details.

## Generate openpi Data
First, create the `training_data` folders in the `policy/pi0` directory:

```
mkdir training_data
```

The default configuration provided by RoboSynChallenge supports data collection from Lerobot 3.0, and we provide a script for converting LeRobot 3.0 data to LeRobot 2.1 in `scripts/convert_lerobot3.0_to_2.1.py`.

```python
python scripts/convert_lerobot3.0_to_2.1.py --repo-id {repo_id} --root /path/to/datasets
```
This script will give you a processed dataset with the same name as the original {repo_id}, but in version 3.0, the dataset will be named {repo_id}_v3.0.
...
Copy all the data you wish to use for training into `training_data/`.

## Write the Corresponding `train_config`

> For our official experiment, we use `pi0_base_robosynchallenge_full`
